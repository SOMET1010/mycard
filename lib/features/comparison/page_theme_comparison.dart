...
class _ThemeComparisonPageState extends State<ThemeComparisonPage> {
  ...
  EventThemeCustomization _createDefaultTheme(String name) => EventThemeCustomization(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: _isSyncEnabled ? 'Désactiver la synchro' : 'Activer la synchro',
          ),
        ],
      ),
      body: Column(
  ...
  Widget _buildDivider() => GestureDetector(
      onHorizontalDragUpdate: (details) {
        setState(() {
          ...
          final size = box.size;
          _splitPosition = (details.localPosition.dx / size.width).clamp(0.2, 0.8);
        });
      },
      child: Container(
        width: 8,
  ...
  Widget _buildThemePanel(EventThemeCustomization theme, String title, bool isLeft) => Container(
  ...
  Widget _buildThemeHeader(EventThemeCustomization theme, String title) => Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Map<String, String> _convertThemeToCustomColors(EventThemeCustomization theme) => theme.colors.map((key, value) => MapEntry(
      key,
  ...
  Widget _buildColorPalette(EventThemeCustomization theme) => Container(
      ...
      child: Row(
        children: theme.colors.entries.map((entry) => Expanded(
            child: Container(
  ...
  Widget _buildThemeActions(EventThemeCustomization theme, bool isLeft) => Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
