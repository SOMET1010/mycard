...
class _PhotoIntegrationWidgetState extends State<PhotoIntegrationWidget>
  ...
  final PhotoIntegrationService _photoService =
      PhotoIntegrationService.instance;
  ...
  Widget _buildTabBar() => DecoratedBox(
    ...
    child: TabBar(
      ...
      tabs: const [
        Tab(icon: Icon(Icons.photo_library), text: 'Sélection'),
        Tab(icon: Icon(Icons.color_lens), text: 'Analyse'),
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: _currentExtraction!.palette
                .map(
                  (color) => Container(
                    width: 60,
                    height: 60,
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 4,
            children: _currentExtraction!.dominantColors
                .map(
                  (color) => Container(
  ...
  Future<void> _analyzeColors() async {
    ...
    try {
      final extraction = await _photoService.analyzePhotoColors(
        _selectedImagePath!,
      );
      setState(() {
  ...
  Future<void> _generateTheme() async {
    if (_selectedImagePath == null || _themeName.isEmpty) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Veuillez sélectionner une photo et nommer le thème'),
          ),
        );
      }
      return;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Erreur génération thème: $e')));
      }
    }
  }
